...
export class DynamicLightManager {
  ...
  /** Source occupant chaque emplacement, pour savoir quand l'ombre change. */
  private readonly assignedPoints: number[] = [];
  private readonly assignedCones: number[] = [];
  private nextId = 1;
  ...
  attach(root: THREE.Object3D, maxActive = this.maxActive, shadowBudget = 0, coneCount = 0): void {
    ...
    this.pool.length = 0;
    this.cones.length = 0;
    this.assignedPoints.length = 0;
    this.assignedCones.length = 0;
  ...
  add(request: LightRequest, follow?: () => Vec3 | null): number {
    ...
    this.lights.push({ ...request, id, age: 0, follow });
    return id;
  }
  ...
  clearTransient(): void {
    for (let i = this.lights.length - 1; i >= 0; i--) {
      if (this.lights[i].duration !== undefined) this.lights.splice(i, 1);
    }
  }
  ...
  update(delta: number, viewer: Vec3): void {
    ...
    for (let slot = 0; slot < this.pool.length; slot++) {
      this.applyPoint(this.pool[slot], this.ranked[slot], slot);
    }
    for (let slot = 0; slot < this.cones.length; slot++) {
      this.applyCone(this.cones[slot], this.rankedCones[slot], slot);
    }
  }

  private applyPoint(target: THREE.PointLight, source: ActiveLight | undefined, slot: number): void {
    if (!source) {
      target.intensity = 0;
      this.assignedPoints[slot] = 0;
      return;
    }
    ...
    target.decay = 1;
    target.intensity = powerOf(source);
    this.refreshShadow(target, source, slot, this.assignedPoints);
  }

  private applyCone(target: THREE.SpotLight, source: ActiveLight | undefined, slot: number): void {
    if (!source || !source.cone) {
      target.intensity = 0;
      this.assignedCones[slot] = 0;
      return;
    }
    ...
    target.intensity = powerOf(source) * 0.7;
    this.refreshShadow(target, source, slot, this.assignedCones);
  }
  ...
  private refreshShadow(
    ...
    if (assigned[slot] !== source.id || moving) {
